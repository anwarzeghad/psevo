...
const IncidentClassifier: React.FC = () => {
  const {
    activeIndex, answers, result, report,
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[268px] flex-shrink-0 flex flex-col bg-white border-r border-[#E2E8F0]">
          ...
          <div className="flex items-center gap-3 p-4 border-b border-[#E2E8F0]">
            ...
            <div>
              <p className="text-[12px] font-black uppercase tracking-[0.1em] text-[#0F1923]"
          ...
          <div className="px-5 py-3 border-b border-[#E2E8F0]">
            <div className="flex justify-between items-center mb-1.5">
              <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-[#9BAABB]">Progress</span>
              <span className="text-[9px] font-black" style={{ color: progressColor }}>{progress}%</span>
            </div>
          ...
          {/* Footer — live UTC clock, no session ID */}
          <div className="p-4 border-t border-[#E2E8F0]">
            <div className="flex items-center justify-between">
              {/* Live clock — prominent and continuous */}
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-[#22C55E] animate-pulse flex-shrink-0" />
                <span className="text-[11px] font-black font-mono text-[#0F1923] tabular-nums">
                  {utcTime}
                </span>
              </div>
            ...
            <p className="text-[8px] text-[#CBD5E1] font-mono mt-1">
            ...
            {!result ? (
              <>
                ...
                <div className="mb-2">
                  ...
                  <h1 className="text-[30px] font-black uppercase leading-none tracking-tight text-[#0F1923] mb-2"
                    style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
                    PSE Classification
                  </h1>
                  ...
                  {STEPS_META.filter((s) => s.key !== "FINAL").map((s, i, arr) => (
                    <React.Fragment key={s.key}>
                      <span className={[
                        ...
                        activeStep === s.key ? "text-[#E8321A]"
                          : s.questionIds.some((id) => id in answers) ? "text-[#3B6FD4]"
                          : "text-[#CBD5E1]",
                      ].join(" ")}>
                        {s.label.split("·")[1]?.trim() ?? s.label}
                      </span>
                      {i < arr.length - 1 && <ChevronRight size={10} className="text-[#E2E8F0] flex-shrink-0" />}
                    </React.Fragment>
                  ))}
                </div>
                ...
                <div className="flex gap-5 items-start">
                  ...
                  <div className="w-44 flex-shrink-0 bg-white border border-[#E2E8F0] rounded-sm shadow-sm overflow-hidden sticky top-4">
                    ...
                    <div className="p-4">
                      <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#3B6FD4] mb-1">Status</p>
                      <p className="text-[17px] font-black leading-snug mb-4 text-[#0F1923]"
                        style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
                      ...
                      <div className="space-y-2">
                        {[
                          ...
                          <div key={label} className={["flex items-center gap-2 text-[10px] transition-colors",
                            active ? "text-[#334155]" : "text-[#CBD5E1]"].join(" ")}>
                            <span className={["w-1.5 h-1.5 flex-shrink-0 rounded-full transition-colors",
                              active ? "bg-[#22C55E]" : "bg-[#CBD5E1]"].join(" ")} />
                            {label}
                          </div>
                        ))}
                      </div>
                      <div className="mt-5 pt-4 border-t border-[#F0F4F8]">
                        <p className="text-[8px] font-bold uppercase tracking-[0.15em] text-[#9BAABB] mb-2">Phase</p>
                    ...
                    {QUESTIONS.map((q, idx) => {
                      ...
                      return (
                        <QuestionCard key={q.id} question={q}
                          active={isActive} answered={isAnswered}
              ...
              report && <ResultScreen result={result} report={report} onReset={reset} />
            )}
          </div>
          ...
          <div className="w-[200px] flex-shrink-0 flex flex-col bg-white border-l border-[#E2E8F0]">
            <div className="p-4 border-b border-[#E2E8F0]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[#E8321A]">Active Thread</span>
              ...
              <p className="text-[8px] text-[#9BAABB] font-mono mt-0.5">PSEVO-7792</p>
            </div>

            {/* Live clock in right panel — large and visible */}
            <div className="px-4 py-3 border-b border-[#E2E8F0] bg-[#F8FAFC]">
              <p className="text-[8px] font-bold uppercase tracking-[0.15em] text-[#9BAABB] mb-1">Live Time</p>
              <p className="text-[14px] font-black font-mono text-[#0F1923] tabular-nums">{utcTime}</p>
            </div>

            <div className="p-4 flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
              <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#9BAABB] mb-3">Event Log</p>
              {answeredCount === 0 ? (
                  ...
                  {QUESTIONS.filter((q) => q.id in answers).map((q) => (
                    <div key={q.id} className="flex items-start gap-2">
                      <span className={["w-1 h-1 rounded-full flex-shrink-0 mt-1.5",
                        answers[q.id] === "YES" ? "bg-[#E8321A]" : "bg-[#3B6FD4]"].join(" ")} />
                      <div>
                        <p className={["text-[9px] font-black font-mono",
                          answers[q.id] === "YES" ? "text-[#E8321A]" : "text-[#3B6FD4]"].join(" ")}>
